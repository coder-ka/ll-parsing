...
検証に成功した場合、パース結果の配列 result.value にトークンを追加し、配列を返します。
...
値は関数で、第1引数は先読みトークンの配列（LL(1)なら長さ1の配列）、第2引数はトークンのソース上の位置を表す情報、第3引数はASTなどの情報を含める為の結果オブジェクトです。
...
結果オブジェクトは、ユーザーが自由に形を決めることができます。例えば、`{ ast: YourLangAST }`という形で指定できます。
...
第2引数は、結果オブジェクトの初期値です。先ほど書いたように、結果オブジェクトの中身はパージングの過程で変更されていきます。
